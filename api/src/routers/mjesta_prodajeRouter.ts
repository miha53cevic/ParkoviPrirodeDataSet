...
const url = '/mjesta_prodaje';

router.get(url, async (req, res) => {
    
    const queryString = "SELECT * FROM MjestoProdajeUlaznice";
    const result = await query(queryString, []);
    if (result.error) return res.status(500).json(createResponse('SERVER ERROR', 'Greška s bazom podataka', result.error));

    return res.status(200).send(createResponse('OK', 'Prodajna mjesta ulaznica za sve parkove unutar baze', result.response?.rows));
});
